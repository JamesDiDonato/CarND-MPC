    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // In order to use polyfit, need to convert waypoint vectors into the
          // Eigen::VectorXd data-type.
          ...
          //Compute the third order polynomial coefficients:
          auto coeffs = polyfit(ptsx_transform, ptsy_transform,3);

          // Approximate the Cross Track Error
          // (True CTE would be shortest distance to the polynomial from (0,0) )
          double cte = polyeval(coeffs,0); // y value of polynomial at vehicle position

          //Approximate the Orientation Error:
          ...
          */
          ...
          //auto msg = "[\"steer\"," + std::to_string(steer_value/deg2rad(25)) + ", throttle," + std::to_string(throttle_value)+"]";
